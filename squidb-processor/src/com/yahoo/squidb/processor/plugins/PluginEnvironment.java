 ...
 * plugins. Options are passed as a comma-separated list of strings to the processor using the key "squidbOptionFlags".
 * <p>
 * All Plugin instances have access to the instance of PluginEnvironment that created them, and can call
 * {@link #getFlag(String)} or {@link #getEnvOptions()} to read any custom flags or options specific to that plugin.
 */
public class PluginEnvironment {

    public static final String PLUGINS_KEY = "squidbPlugins";
    public static final String OPTIONS_KEY = "squidbOptionFlags";
     ...
     */
    public static final String OPTIONS_DISABLE_DEFAULT_IMPLEMENTS_HANDLING = "squidb_disableImplements";

    /**
     * Flag for disabling the default copying of static methods and model methods from the spec to the model class
     */
    public static final String OPTIONS_DISABLE_DEFAULT_METHOD_HANDLING = "squidb_disableModelMethod";

    /**
     * Flag for disabling the default copying of public static final fields as constants to the generated model
     */
    public static final String OPTIONS_DISABLE_DEFAULT_CONSTANT_COPYING = "squidb_disableConstantCopying";

    /**
     * Flag for disabling the in-memory default content values used as for fallback values in empty models
     */
    public static final String OPTIONS_DISABLE_DEFAULT_CONTENT_VALUES = "squidb_disableDefaultContentValues";

    /**
     * Flag for disabling the convenience getters and setters generated by default with each property
     */
    public static final String OPTIONS_DISABLE_DEFAULT_GETTERS_AND_SETTERS = "squidb_disableGettersAndSetters";
    ...
    public PluginEnvironment(AptUtils utils, Map<String, String> envOptions) {
        ...
        this.squidbOptions = parseOptionFlags();
        initializeDefaultPlugins();
        initializePluginsFromEnvironment();
    }

    private Set<String> parseOptionFlags() {
        Set<String> result = new HashSet<String>();
        if (envOptions.containsKey(OPTIONS_KEY)) {
            String flagsString = envOptions.get(OPTIONS_KEY);
            String[] allFlags = flagsString.split(SEPARATOR);
            Collections.addAll(result, allFlags);
        }
        return result;
    }
    ...
    private void initializePluginsFromEnvironment() {
        String plugins = envOptions.get(PLUGINS_KEY);
        ...
        try {
            ...
            if (pluginName.contains(":")) {
                ...
                if (nameAndPriority.length != 2) {
                    utils.getMessager().printMessage(Diagnostic.Kind.ERROR, "Error parsing plugin and priority " + pluginName);
                } else {
                    ...
                    if (priority == null) {
                        utils.getMessager().printMessage(Diagnostic.Kind.WARNING, "Unrecognized priority string " + priorityString
            ...
            utils.getMessager()
                    .printMessage(Diagnostic.Kind.WARNING, "Unable to instantiate plugin " + pluginName + ", reason: " + e);
        }
    }
    ...
    public boolean getFlag(String flag) {
        return squidbOptions.contains(flag);
    }
