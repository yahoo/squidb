...
runAndroidTestWithBinding () {
    ...
    python3 ./scripts/adb-wrapper.py "adb shell am instrument -w -e squidb_binding $1 com.yahoo.squidb.test/com.yahoo.squidb.android.SquidInstrumentationTestRunner"
    testResult=$?
    if [ ! $testResult -eq 0 ]
    then
